...
serve_read(envid_t envid, union Fsipc *ipc)
{
    ...
    // Actual size to read.
    size_t rdsz = req->req_n < PGSIZE-8 ? req->req_n : PGSIZE-8;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
    ...
    size_t wrsz = req->req_n < PGSIZE-8 ? req->req_n : PGSIZE-8;
...
serve_flush(envid_t envid, struct Fsreq_flush *req)
{
	...
	if ((r = openfile_lookup(envid, req->req_fileid, &o)) < 0){
		panic("openfile lookup failed with r = [%d]",r);
		return r;
		}
	file_flush(o->o_file);

	if (debug)
		cprintf("serve_flush returns %08x %08x\n", envid, req->req_fileid);
	return 0;
}
