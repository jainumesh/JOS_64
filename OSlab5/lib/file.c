...
fsipc(unsigned type, void *dstva)
{
	...
	//cprintf("fsenv = [%d]   [%p] dstva [%p]",fsenv, &fsenv,dstva);
	if (fsenv == 0){
	...
	//cprintf("Calling IPC_SEND with  env #[%d] and type is [%d]\n",fsenv,type);
	ipc_send(fsenv, type, &fsipcbuf, PTE_P | PTE_W | PTE_U);
	result= ipc_recv(NULL, dstva, NULL);
	//cprintf("fsenv = [%d]   [%p]",fsenv, &fsenv);
	return result;
}
