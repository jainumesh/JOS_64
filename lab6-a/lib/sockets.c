...
fd2sockid(int fd)
{
	...
	if (sfd->fd_dev_id != devsock.dev_id)
		return -E_NOT_SUPP;
	cprintf("Umesh: Retunrning socket \n");	
...
accept(int s, struct sockaddr *addr, socklen_t *addrlen)
{
	...
	cprintf("Umesh: Entering accept\n");	
	if ((r = fd2sockid(s)) < 0){
		cprintf("Umesh: Returning as fdsockid less than 0 [%d]",r);	
		return r;
	}
	if ((r = nsipc_accept(r, addr, addrlen)) < 0){
		cprintf("Umesh: Returning as nsipcaccept returns failure [%d]",r);
		return r;
	}
	cprintf("Umesh: Returning successfully from accept [%d]",r);
...
{
	int r;
	cprintf("Umesh: calling Connect from here\n");
	if ((r = fd2sockid(s)) < 0)
		return r;
