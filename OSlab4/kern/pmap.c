...
x64_vm_init(void)
{
	...
	// Your code goes here:
    boot_map_region(pml4e, UPAGES, npages * sizeof(struct PageInfo), PADDR(pages), PTE_USER);
	...
	// LAB 3: Your code here.
	boot_map_region(pml4e, UENVS, NENV * sizeof(struct Env), PADDR(envs), PTE_USER);
	...
	mem_init_mp();
	boot_map_region(pml4e, KERNBASE, npages*PGSIZE, 0, PTE_USER);
